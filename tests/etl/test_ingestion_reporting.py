...
def test_ingestion_reports_skipped_rows():
    """PR-08: Ingestion should collect and report rows that fail validation."""
    input_rows = [
        {"candidate_id": "good1", "resume_path": "resumes/good1.pdf"},
        {"candidate_id": "bad1"},  # missing resume_path
        {"resume_path": "resumes/bad2.pdf"},  # missing candidate_id
        {"candidate_id": "good2", "resume_path": "resumes/good2.pdf", "email": "test@example.com"},
    ]
    ...
    for proc_entry in processed:
        ...
        assert "resume_path" in proc_entry
